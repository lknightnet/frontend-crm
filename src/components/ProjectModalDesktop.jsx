...
const ProjectModalDesktop = ({project, onClose}) => {
    ...
    const fetchProjectUsers = async (query) => {
        setIsLoading(true);
        try {
            const response = await authFetch('http://localhost:8010/api/user/get/like', {
                method: 'POST',
                headers: {
    ...
    const handleCreateProject = async () => {
        ...
        try {
            const response = await authFetch('http://localhost:8012/api/task/create', {
                method: 'POST',
    ...
    const handleSave = async (id) => {
        ...
        const url = isNew
            ? "http://localhost:8012/api/lists/create"
            : "http://localhost:8012/api/lists/update";
    ...
    useEffect(() => {
        ...
        const fetchProjectLists = async () => {
            try {
                const response = await authFetch(`http://localhost:8012/api/lists/get/project/${project.id}`);
    ...
    const handleDelete = async (id) => {
        ...
        if (isPersisted) {
            try {
                const response = await authFetch(`http://localhost:8012/api/lists/delete/${id}`, {
                    method: 'GET',
                });
